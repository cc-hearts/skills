#!/usr/bin/env node
/*
 * 宣 Xuan · verify v0.1 —— 每条铁律至少一个自动检查
 ...
 * 例外：那一行行尾加注释 xuan:allow 并写明理由；verify.mjs、
...
if (violations.length > 0) {
  console.error(`\n✗ 宣 verify：${violations.length} 处违反铁律（检查了 ${checked} 个文件）\n`);
  for (const v of violations) console.error(v + '\n');
  console.error('确需例外：那一行行尾加注释 xuan:allow，并写明理由。');
  process.exit(1);
} else {
  console.log(`✓ 宣 verify：${checked} 个文件全部通过。verify 只证明没违反铁律，好不好看请对照主张自查。`);
}
